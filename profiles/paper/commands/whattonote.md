...
The read-only, advisory sibling of `/note`. It never writes, logs, or edits any file; the only
output is a short bullet summary of what you would capture and where each piece would go. Think
"what would `/note` find here if I ran it now?" Useful when several capturable things have piled
...
**3. Classify each the way `/note` does.** For every candidate name the destination, the command,
and the gating:

| If the item is... | Destination | Command | Gating |
|---|---|---|---|
| a specific run / result | detail leaf + `experiments.md` | `/logexp` | low |
| ...and it looks paper-critical | + `experiments_important.md` | `/logexp` | medium, explicit ask |
| durable methodology / a gotcha | `technical_notes/` + its index | `/technote` | low new, medium edit |
| a thematic paper claim | `paper_narrative.md` | `/papernote` | high, sentence-by-sentence |
...
conflicting info). Index level only; the exhaustive sweep is `/paper-audit`.
...
next step (e.g. "run /note on items 1-3; item 4 is already covered"). If nothing is worth
...
happens inside `/logexp` / `/papernote` when you choose to run them.
