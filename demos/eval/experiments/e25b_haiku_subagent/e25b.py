...
Identical corpus and queries as e25; only difference is
MEMORY_RECALL_SUBAGENT_MODEL=claude-haiku-4-5-20251001 set in env, so
the multi arm uses Haiku for capture and search expansion.

Question: does Haiku-as-subagent preserve the recall lift seen in e25
at meaningfully lower cost and latency?
...
async def main_async(args: argparse.Namespace) -> int:
    print("=" * 64)
    print(f"e25b: multi-vector with sub-agent model = {os.environ.get('MEMORY_RECALL_SUBAGENT_MODEL', '(default)')}")
    print("=" * 64)
    ...
    (RESULTS_DIR / "e25b_results.json").write_text(json.dumps(results, indent=2))
