"""e01b — Same task as e01 with EXPLICIT memory tool schema in prompt.

Background: e01's with_memory arm called memory_capture_batch with
`content` instead of the required `body` field, got silently rejected,
and fell back to filesystem. The cost comparison was contaminated.

This run gives the agent the exact field schema in the phase-1 prompt
and instructs it to verify via memory_status. Everything else is the
same as e01.
"""
...
LEAVE NOTES so the next agent can answer. You have access to
`mcp__memory-graph__*` tools. Use them. The required schema for
`memory_capture_batch` is:

```
{
  "notes": [
    {
      "title": "short title, e.g. 'module_00: off-by-one in pagination'",
      "summary": "one-sentence summary of the bug",
      "body": "longer description: which file, which method, the buggy line, and what the fix would be",
      "kind": "bug",
      "tags": ["module_00", "off-by-one", "pagination"]
    },
    ...
  ]
}
...
REQUIRED FIELDS PER NOTE: `title`, `summary`, `body`, `kind`. Do NOT
use `content` — the field is `body`. Do NOT omit `kind` — use the
string "bug" for every note in this task. The `tags` field MUST
include the module name (e.g. `"module_00"`) so the next agent can
filter notes by module.

After your batch call, IMMEDIATELY verify by calling
`memory_status` — `total_nodes` should be 30. If it's less, you
made a schema error; correct it and try again.

Read all 30 modules carefully. For each one, capture one memory note.
"""
...
async def run_arm(arm: str) -> dict[str, Any]:
    ...
    workdir = WORK_BASE / f"e01b-{arm}-{ts}"
    if workdir.exists():
...
async def main_async() -> int:
    print("=" * 64)
    print("e01b: Same task, explicit memory schema in prompt")
    print("=" * 64)
    no_mem = await run_arm("no_memory")
    with_mem = await run_arm("with_memory")
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    (RESULTS_DIR / "e01b_results.json").write_text(
